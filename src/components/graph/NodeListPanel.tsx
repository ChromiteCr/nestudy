 ...
 * 分组为专业方向 / 反思 / 已连接叙事线 / 尚未连接——反思单独一组：它们绕父活动卫星定位，
 * 没有叙事线也不代表"没归属"，混进"尚未连接"会误导用户去手动连线。
 */
export function NodeListPanel({ nodes, narrativeEdges, onSelect, onClose }: NodeListPanelProps) {
  ...
  const reflections = nodes.filter((n) => n.kind === 'reflection')
  const others = nodes.filter((n) => n.kind !== 'major' && n.kind !== 'reflection')
  ...
  return (
    <div
      className={`flex max-h-[60dvh] w-64 flex-col gap-2 rounded-xl border bg-popover p-3 text-popover-foreground shadow-lg ring-1 ring-border ${
    ...
    >
      ...
      <div className="flex flex-col gap-3 overflow-y-auto">
        ...
        <Group title={`反思 · ${reflections.length}`} items={reflections} onSelect={onSelect} />
